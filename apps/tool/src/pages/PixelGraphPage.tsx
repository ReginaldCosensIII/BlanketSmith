...
export const PixelGraphPage: React.FC<PixelGraphPageProps> = ({
    ...
    // [GAUGE-001] Derive the Y-axis aspect ratio for the editor grid. Falls back to 1 (square cells).
    const gaugeStitches = Number(project?.settings?.stitchesPerUnit ?? 0);
    const gaugeRows = Number(project?.settings?.rowsPerUnit ?? 0);
    const applyGaugeToEditor = project?.settings?.visuals?.applyGaugeToEditor ?? false;
    const editorAspectRatio = (applyGaugeToEditor && gaugeStitches > 0 && gaugeRows > 0)
