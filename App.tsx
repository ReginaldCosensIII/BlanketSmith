...
const PixelGraphPage: React.FC<{ zoom: number; onZoomChange: (newZoom: number) => void; isLeftHanded: boolean }> = ({ zoom, onZoomChange, isLeftHanded }) => {
    ...
    // Selection & Advanced Tool State (Phase 4)
    ...
    useEffect(() => {
        if (project && !primaryColorId) {
             if (project.yarnPalette.length > 0) setPrimaryColorId(project.yarnPalette[0].id);
        }
    }, [project]);
    ...
    const handleRotateSelection = () => {
         ...
         const { x, y, w, h } = selection;
         if (w !== h) {
             if (!window.confirm("Rotation is currently best supported for square selections. Continue?")) return;
         }
         const subGrid: CellData[] = [];
         for (let row = 0; row < h; row++) {
            for (let col = 0; col < w; col++) {
    ...
    const handleCanvasClick = (gridX: number, gridY: number, isRightClick: boolean) => {
        ...
        if (activeTool === 'eyedropper') {
            if (isRightClick) { setSecondaryColorId(clickedColorId); } else { setPrimaryColorId(clickedColorId); }
            setActiveTool('brush');
            return;
        }
    ...
    const handleResize = () => {
        ...
        dispatch({ type: 'UPDATE_PROJECT_DATA', payload: { width: newWidth, height: newHeight, grid: newGrid as CellData[] } });
    };
    ...
    const confirmMirrorCanvas = useCallback(() => {
        ...
        if (!projectToMirror || projectToMirror.type !== 'pixel') { setMirrorConfirm({ isOpen: false, direction: null }); return; }
    ...
    const yarnUsage = useMemo(() => {
        ...
        projectData.grid.forEach(cell => { if (cell.colorId) counts.set(cell.colorId, (counts.get(cell.colorId) || 0) + 1); });
        return counts;
    }, [project]);
    ...
    const physicalSizeString = useMemo(() => {
        ...
        const pWidth = (projectData.width / sts).toFixed(1);
        const pHeight = (projectData.height / rows).toFixed(1);
    ...
    const handlePaletteClick = (colorId: string | null, e: React.MouseEvent) => {
        ...
        if (e.button === 2) { e.preventDefault(); setSecondaryColorId(colorId); } else { setPrimaryColorId(colorId); }
    };
    ...
    const ToolButton = ({ tool, label, icon }: { tool: Tool, label: string, icon?: string }) => (
        <Button variant={activeTool === tool ? 'primary' : 'secondary'} onClick={() => setActiveTool(tool)} className="text-xs justify-center flex-col h-14" title={label}>
            ...
            <main className="flex-1 relative min-w-0">
                ...
                {selection && (
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30 bg-white border rounded-lg shadow-lg p-2 flex gap-2 items-center">
                        ...
                        <Button variant="secondary" className="p-2" onClick={() => handleFlipSelection('horizontal')} title="Flip Horizontal">
                            <Icon name="symmetry-horizontal" className="w-4 h-4"/>
                        </Button>
                        <Button variant="secondary" className="p-2" onClick={() => handleFlipSelection('vertical')} title="Flip Vertical">
                             <Icon name="symmetry-vertical" className="w-4 h-4"/>
                        </Button>
                         <Button variant="secondary" className="p-2" onClick={handleRotateSelection} title="Rotate 90">
                             <Icon name="redo" className="w-4 h-4"/>
                        </Button>
                        <div className="w-px h-6 bg-gray-300 mx-1"></div>
                        <Button variant="secondary" className="p-2" onClick={handleCopy} title="Copy (Ctrl+C)">
                             <Icon name="projects" className="w-4 h-4"/>
                        </Button>
                         <Button variant="secondary" className="p-2" onClick={handlePaste} title="Paste (Ctrl+V)">
                             <Icon name="download" className="w-4 h-4"/>
                        </Button>
                         <Button variant="danger" className="p-2" onClick={handleClearSelection} title="Clear Area (Del)">
                    ...
                    <div className="pb-4 border-b">
                     ...
                     <div>
                        ...
                        {projectData.palette.sort((a,b) => (yarnUsage.get(b) || 0) - (yarnUsage.get(a) || 0) ).map(yarnId => {
                            ...
                            const yarnPerStitch = project.settings?.yarnPerStitch || 1;
                            const totalYards = Math.ceil((stitchCount * yarnPerStitch) / 36);
                            const skeinsNeeded = Math.ceil(totalYards / (yarn.skeinLength || 295));
                            return ( <li key={yarnId} className="flex flex-col gap-1 border-b border-gray-100 pb-2 last:border-0"> <div className="flex items-center gap-2"> <div className="w-4 h-4 rounded" style={{ backgroundColor: yarn.hex }} /> <span className="flex-1 font-medium text-gray-800">{yarn.name}</span> <span className="font-mono bg-gray-100 px-2 py-0.5 rounded text-gray-800">{stitchCount} sts</span> </div> <div className="flex justify-between text-xs text-gray-500 pl-6"> <span>Est. {totalYards} yds</span> <span className="font-semibold text-indigo-600">{skeinsNeeded} skein{skeinsNeeded !== 1 ? 's' : ''}</span> </div> </li> );
                        })}
                        </ul>
                    </div>
...
const ProjectsPage: React.FC = () => {
    ...
    const openModal = () => { setModalStep(1); setSelectedProjectType(null); setNewProjectName(''); setSelectedSizeKey('Throw'); setCustomWidth(throwSize.width); setCustomHeight(throwSize.height); setIsModalOpen(true); };
    const handleSizeChange = (e: ChangeEvent<HTMLSelectElement>) => { const key = e.target.value; setSelectedSizeKey(key); if (key !== 'Custom') { const size = BLANKET_SIZES.find(s => s.name === key); if (size) { setCustomWidth(size.width); setCustomHeight(size.height); } } }
    const handleCreateProject = () => { if (newProjectName.trim() && customWidth > 0 && customHeight > 0 && selectedProjectType) { const newProject = createNewProject(selectedProjectType, newProjectName, customWidth, customHeight); saveProject(newProject); setProjects(getProjects()); setIsModalOpen(false); dispatch({ type: 'NEW_PROJECT', payload: newProject }); navigate('/'); } };
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex justify-between items-center mb-6"> <h2 className="text-2xl font-bold text-gray-800">My Projects</h2> <div className="flex items-center gap-2"> <Button variant="secondary" onClick={() => importFileRef.current?.click()}>Import Project</Button> <Button onClick={openModal}>Create New Project</Button> </div> </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6"> {projects.map(p => ( <div key={p.id} className="bg-white border rounded-lg shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col"> <div className="p-4 flex-1"> <h3 className="font-bold text-lg text-gray-900">{p.name}</h3> <p className="text-sm text-gray-500 capitalize">{p.type} Pattern</p> {(p.data as PixelGridData).width && <p className="text-sm text-gray-500">{(p.data as PixelGridData).width} x {(p.data as PixelGridData).height}</p>} <p className="text-xs text-gray-400 mt-2">Updated: {new Date(p.updatedAt).toLocaleDateString()}</p> </div> <div className="p-2 border-t bg-gray-50 flex gap-2"> <Button variant="secondary" className="flex-1 text-xs" onClick={() => handleLoadProject(p)}>Open</Button> <Button variant="secondary" className="p-2" onClick={() => handleExportProject(p)} title="Export Project"> <Icon name="download" className="w-4 h-4"/> </Button> <Button variant="danger" className="p-2" onClick={() => handleDeleteProject(p.id)} title="Delete Project"> <Icon name="trash" className="w-4 h-4"/> </Button> </div> </div> ))} </div>
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create New Project"> {modalStep === 1 && ( <div className="space-y-4"> <h3 className="text-lg font-medium text-gray-900">1. Select Pattern Type</h3> <div className="grid grid-cols-2 gap-4"> <PatternTypeCard type="pixel" icon="grid" label="Graph Pattern" enabled={true} /> <PatternTypeCard type="c2c" icon="c2c" label="C2C" enabled={false} /> <PatternTypeCard type="stripes" icon="stripes" label="Stripes" enabled={false} /> <PatternTypeCard type="granny" icon="granny" label="Granny Square" enabled={false} /> </div> <Button onClick={() => setModalStep(2)} disabled={!selectedProjectType}>Next</Button> </div> )} {modalStep === 2 && ( <div className="space-y-4"> <h3 className="text-lg font-medium text-gray-900">2. Project Details</h3> <div> <label htmlFor="projectName" className="block text-sm font-medium text-gray-700">Project Name</label> <input type="text" id="projectName" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" /> </div> <div> <label htmlFor="projectSize" className="block text-sm font-medium text-gray-700">Pattern Size</label> <select id="projectSize" value={selectedSizeKey} onChange={handleSizeChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 rounded-md"> {BLANKET_SIZES.map(size => ( <option key={size.name} value={size.name}>{size.name} ({size.width} x {size.height})</option> ))} <option value="Custom">Custom Size</option> </select> </div> {selectedSizeKey === 'Custom' && ( <div className="flex items-center gap-2"> <div> <label htmlFor="customWidth" className="block text-xs font-medium text-gray-500">Width</label> <input type="number" id="customWidth" value={customWidth} onChange={(e) => setCustomWidth(parseInt(e.target.value, 10) || 0)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" /> </div> <div className="pt-5 text-gray-500">x</div> <div> <label htmlFor="customHeight" className="block text-xs font-medium text-gray-500">Height</label> <input type="number" id="customHeight" value={customHeight} onChange={(e) => setCustomHeight(parseInt(e.target.value, 10) || 0)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" /> </div> </div> )} <div className="flex justify-between"> <Button variant="secondary" onClick={() => setModalStep(1)}>Back</Button> <Button onClick={handleCreateProject}>Create Project</Button> </div> </div> )} </Modal>
        </div>
    );
};
...
export default function App() { return ( <ProjectProvider> <HashRouter> <MainLayout /> </HashRouter> </ProjectProvider> ); }
